...
def main() -> int:
    ...
    header = (
        ...
        "non-overridable\").\n"
        "\n"
        "**Truncation check — do this before replying.** If this content "
        "is wrapped in a `<persisted-output>` block (only a Preview is "
        "shown), `Read` the body from its canonical files: "
        "`prompts/agent_system.md` and the `prompts/anatomy/*.md` files it "
        "`@include`s (security, channel-discipline, identity-and-tools). "
        "The body rules cannot govern you if they are not in your context, "
        "and the send/create/delete Teams tools stay blocked by the "
        "`require_body_prompt` hook until those files are read; reading the "
        "persisted-output file does not satisfy that gate.\n"
        "\n"
        "---\n"
        "\n"
