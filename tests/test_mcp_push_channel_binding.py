...
class TestBotPollPathBinds:
    """Bot-mode inbound (``_background_poll_bot``) routes through
    ``_push_channel_notification`` so the binding wiring is shared.
    This test pins that invariant so a future bot-mode refactor that
    bypasses the shared helper would fail loudly.
    """

    @pytest.mark.asyncio
    async def test_bot_message_via_shared_helper_creates_binding(
        self, monkeypatch, sponsor_record
    ) -> None:
        from entrabot import mcp_server
        ...
        # This is the exact call shape from _background_poll_bot
        # (mcp_server.py: `await _push_channel_notification(msg,
        ...
        await mcp_server._push_channel_notification(
            {
                ...
                "from": "sarah@example.com",
                "sender_id": "sponsor-uid-1",
                "content": "via bot gateway",
                "sent_at": _now_iso(),
            },
            chat_id="19:botchat@thread.v2",
        )

        b = get_bindings().lookup("sponsor-uid-1")
        assert b is not None
        assert b.chat_id == "19:botchat@thread.v2"
        assert b.message_id == "bot-m-1"
