...
Read the listed documentation before drafting code or a plan that touches the topic.

| Topic | Required reading |
|---|---|
| OAuth, OBO, PKCE, redirect URIs, JWT | `docs/platform-docs/agent-id-blueprints-and-users.md` |
| Agent Identity, Blueprint, or Agent User | `docs/platform-docs/agent-id-blueprints-and-users.md` |
| MSAL, delegated auth, token acquisition | `docs/platform-docs/delegated-auth.md` and `docs/architecture/identity-and-token-flow.md` |
| Agent User onboarding or consent | `docs/platform-docs/entra-agent-users.md` |
| Teams Graph and chat tools | `docs/platform-docs/teams-graph-api.md` |
| Files Graph | `docs/platform-docs/files-graph-api.md` |
| MCP transports and lifecycle | `docs/platform-docs/mcp-hosts-and-transports.md` and `docs/architecture/mcp-runtime.md` |
| Platform credential storage | `docs/platform-docs/platform-{macos,linux,windows}.md` |
...
- `CredentialStore` implementations under `platform/` keep certificate private keys in the OS
  keystore through `keyring`.
- The three-hop Teams token flow lives in `acquire_agent_user_token`; Hop 1 uses the certificate
  JWT assertion, and Graph/Work IQ HTTP calls use `httpx` with bounded timeouts.
- Token renewal combines eager `_ensure_valid_token()` checks with lazy 401 retry through
  `_with_token_retry()`.
- The background channel poll and explicit reply-watching paths must retain separate deduplication
  state; see Learning #27.
- Every agent action touching a resource must emit its audit event before returning.
- Type-annotate all function signatures, and mirror source layout in the test tree.
...
1. **If `bootstrap_session()` succeeds and `mind_contract_available` is `true`**:
   ...
   - Treat the mind contract as authoritative for your behavior; it layers on
     top of this `AGENTS.md` (body rules remain non-overridable, per the
...
# Run with Claude Code channel notifications
