...
1. **`_init_auth()`** — authenticates by credential presence, not by `ENTRABOT_MODE`. When a Blueprint app ID + tenant ID are configured and `ENTRABOT_SKIP_PROVISIONING` is false, it tries the three-hop Agent User fast path first; on failure or bypass it falls back to MSAL delegated auth when `ENTRABOT_CLIENT_ID` is set, landing in `UNAUTHENTICATED` if neither path succeeds. `ENTRABOT_MODE` is validated but not currently consumed here. See [Identity and Token Flow](identity-and-token-flow.md) for the flow itself and the state machine it drives.
2. **`_init_poll()`** — loads any persisted `watched_chats` file and starts the Teams poll task if at least one chat is already watched; starts email poll, chat auto-discovery, and the daily summary scheduler only when the resulting session is an Agent User session.
