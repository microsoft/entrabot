...
`scripts/entra_provisioning.py::ensure_app_registration()` creates a long-lived **client secret** on the `EntraBot Agent ID Provisioner` app registration and writes it verbatim to **`.entrabot-state.json`** at the repo root:
...
- **Exfiltrated by the first compromise** — a single malicious Python package or VS Code extension gets `.entrabot-state.json` and from there can mint Graph tokens with **Application.ReadWrite.All, DelegatedPermissionGrant.ReadWrite.All, User.ReadWrite.All, AgentIdentity.\*, AgentIdUser.\***. That combination can create agent users, impersonate them, grant arbitrary consent, and read the entire directory.
...
Client secrets are secrets. `.entrabot-state.json` is a file. The rule is clear.
...
2. **Generate the cert on first bootstrap** using `cryptography` (RSA 2048, CN=`entrabot-provisioner`, 365-day validity). Register the public cert on the app via `az ad app credential reset --cert @<tempfile> --append`. Discard the tempfile immediately.
3. **Store the private key in macOS Keychain** via `keyring` — exactly like `setup.sh` already does for the Blueprint cert. Service name convention: `entrabot-provisioner-cert`, account = tenant id.
4. **Purge `PROVISIONER_CLIENT_SECRET` from `.entrabot-state.json`** at the same moment. Never write it again.
...
The file layout, naming, and migration path are all transferable to entrabot with the service name changed (`entrabot-provisioner-cert` vs `persona-sati-provisioner-cert`) and the state-file name changed (`.entrabot-state.json` vs `.persona-sati-state.json`).

## Migration requirement (non-negotiable)

This is not a green-field change. Existing developer machines almost certainly have a live `PROVISIONER_CLIENT_SECRET` in `.entrabot-state.json`. The fix MUST include a one-shot migration:

1. Read the existing `PROVISIONER_CLIENT_SECRET` from `.entrabot-state.json` (needed only to verify the app is live — do not use it for anything else).
...
5. **Remove `PROVISIONER_CLIENT_SECRET`** from `.entrabot-state.json`.
...
- **Do not rename the app registration.** Keep `EntraBot Agent ID Provisioner`; adding a cert is a credential-type change, not an identity change.
...
- [ ] `security find-generic-password -a <tenant-id> -s entrabot-provisioner-cert` finds the key.
- [ ] `.entrabot-state.json` contains no `PROVISIONER_CLIENT_SECRET` field after the migration.
