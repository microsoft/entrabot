...
| 006 | Remove the Teams Bot Gateway auth mode | **Accepted — shipped** | The `bot` mode and `src/entrabot/bot/` are deleted. Bot mode bypassed Agent Identity (off-thesis) and was superseded by Microsoft Agent 365's managed AI teammate. `ENTRABOT_MODE=bot` now fails loud via `RemovedModeError`. Two modes remain: `agent_user` and `delegated`. |
