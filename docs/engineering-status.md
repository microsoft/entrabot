...
- **ENTRABOT harness** (branch `users/jaly/entrabot-harness`) — a single-agent Copilot harness that routes Microsoft Teams through a Copilot session (the `entrabot` CLI), an alternative front-end to the MCP body. Guided, idempotent `entrabot init` (resumes an already-provisioned directory instead of re-minting). Per-caller tool permissions via a three-column `/permissions` matrix (cli · sponsor · guest); local terminal input is framed `[cli]` and echoed, so the agent replies in the terminal, not Teams. Inbound Teams / scheduled messages **steer** an in-progress turn (`mode="immediate"`) rather than interrupt it or wait for it to finish. Sponsorship is the **Entra Agent-Identity sponsor relationship** (the same source the body gates on) — read via the pre-existing core `identity.sponsors` read API; `/users` and `entrabot users` *list* the agent's sponsors (read-only — add/remove is done in Entra directly), and there is no harness-local sponsor list. The harness does not self-load the entrabot MCP body (its tools are excluded to avoid duplicating the reply path), and `/mcp` shows what the Copilot SDK actually discovered. Core footprint is minimal/behaviour-preserving (layered config loading in `config.py`); everything else lives under `src/entrabot/harness/`.
