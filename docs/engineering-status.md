...
- **Harness: CLI caller-class + `/users` role matrix** (2026-06-24, branch `users/jaly/entrabot-harness`) — the local terminal operator is now its own **`cli`** caller-class (was folded into `sponsor`), so `/permissions` is a **three-column** matrix (cli · sponsor · guest; `cli_all` defaults on). Fixes two CLI bugs: the operator's own typed line is now echoed to the transcript, and local input is framed `[cli]` (+ a `channel-discipline.md` rule + system-message note) so the agent replies in the terminal instead of Teams. Recipients gain a per-user **Role** (`Recipient.sponsor`, stored as `ENTRABOT_HUMAN_SPONSOR_FLAGS`): everyone defaults **Guest**, and `session._load_sponsors` now honors only flagged users (the harness caller-class gate — distinct from the Entra Agent-Identity sponsor relationship that governs the MCP body's own `add_member`/`share_file` gates). New `/users` role matrix (`edit_users` in both UIs) to elevate/demote; `entrabot users sponsor|guest <email>` + `/users sponsor|guest` shortcuts; `users list` shows Type + Role. +~40 tests.
