...
- **ENTRABOT harness** (branch `users/jaly/entrabot-harness`) — a single-agent Copilot harness that routes Microsoft Teams through a Copilot session (the `entrabot` CLI), an alternative front-end to the MCP body. Guided, idempotent `entrabot init` (resumes an already-provisioned directory instead of re-minting). Per-caller tool permissions via a three-column `/permissions` matrix (cli · sponsor · guest); local terminal input is framed `[cli]` and echoed, so the agent replies in the terminal, not Teams. Inbound Teams / scheduled messages **steer** an in-progress turn (`mode="immediate"`) rather than interrupt it or wait for it to finish. Sponsorship is the **Entra Agent-Identity sponsor relationship** — managed via `/users` and `entrabot users sponsor|guest`, building on a new additive sponsor read/write API in core `identity/sponsors.py` (no harness-local sponsor list). The harness does not self-load the entrabot MCP body (its tools are excluded to avoid duplicating the reply path), and `/mcp` shows what the Copilot SDK actually discovered. Core footprint is additive/behaviour-preserving (layered config loading in `config.py`, the sponsor-write API); everything else lives under `src/entrabot/harness/`.
