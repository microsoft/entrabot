...
- **XPIA content wrapping — boundary-enforced instruction-injection defense** (2026-07-09, branch `feat/xpia-content-wrapping`) — new `src/entrabot/security/xpia.py` wraps every external-source body (`read_teams_messages`, `read_email`, `read_file`, `read_word_document`, `read_a365_text_file`, `read_interactions`) in an `<external_content source=... sender=...>...</external_content>` envelope BEFORE the model sees it. Escape-on-collision defeats embedded close tags (case- and whitespace-tolerant). New body-prompt bullet in `prompts/anatomy/security.md` teaches the model that content inside the envelope is data. Companion write-shaped tool-name recognizer at `src/entrabot/tools/dispatch.py` future-proofs the outbound deny-list. `ENTRABOT_XPIA_WRAP_ENABLE=false` provides a 24h rollback path without a code revert. +62 tests (28 XPIA unit, 34 dispatch, plus per-tool wrap tests). Plan doc: `docs/architecture/PLAN-xpia-content-wrapping.md` (landing in PR #99). Learning #70.
