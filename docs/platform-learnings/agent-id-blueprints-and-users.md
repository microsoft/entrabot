...
│                                                                  │
│  This app reg is what Claude Code / Cursor / VS Code / any       │
│  spec-compliant MCP client uses as its OAuth client_id when      │
│  doing dynamic registration fallback. Persona-sati's DCR shim    │
│  returns this app's appId to the MCP client.                     │
└──────────────────────────────────────────────────────────────────┘
```
...
The MCP Client app reg is **delegated**: it requests the Blueprint's `access_agent` scope, the user consents, and the resulting access token has `aud = <blueprint-appId>`. The MCP server validates this token (it's already what `JWTValidator` does in persona-sati) and accepts it.
...
Phase 2B in the persona-sati engineering-status: persona-sati becomes its own OAuth 2.1 AS, implementing `/authorize`, `/token`, and PKCE state, brokering to Entra on the backend via the existing FIC three-hop. This works but is significantly more code; only consider it if (a) you cannot create a second app registration, or (b) you need the MCP server to be portable across IdPs (not Entra-specific).
...
The persona-sati Blueprint is created with `@odata.type=agentIdentityBlueprint`, which makes it an Agent Identity Blueprint, not an ordinary application. Per [agent-oauth-protocols](https://learn.microsoft.com/en-us/entra/agent-id/agent-oauth-protocols):
...
What Task 0 missed was Step 2.5: **before assuming the Blueprint can be the OAuth client, check whether Microsoft allows that at all.** The codex consult that informed the plan was reasoning from RFC 8414 + MCP spec, not from Microsoft's Agent Identity post-GA semantics. RFC 8414 says nothing about Agent Identities; it's IdP-agnostic. Microsoft's enforcement happens above the protocol layer, in the Graph API and `az` CLI.
...
```bash
az ad app create \
  --display-name "Persona-Sati MCP Client" \
...
Configure it as a delegated client of the Persona-Sati Blueprint's `access_agent` (or `access`) scope:

```bash
az ad app permission add \
  --id <mcp-client-appId> \
  --api <persona-sati-blueprint-appId> \
  --api-permissions <access-scope-id>=Scope
```

Update the persona-sati DCR shim's `/register` route to return `<mcp-client-appId>` as the `client_id`. Add `<mcp-client-appId>` to `PERSONA_SATI_ALLOWED_APP_IDS` so `JWTValidator` accepts the resulting tokens (the tokens will have `aud = <blueprint-appId>` because the requested scope is `api://<blueprint-appId>/access_agent`, and the `azp` claim will be `<mcp-client-appId>`).

Net flow:

1. Claude Code's MCP client gets 401 from persona-sati MCP, follows WWW-Authenticate to PRM.
...
4. MCP client does dynamic-registration fallback at persona-sati's `/register` shim, gets back `<mcp-client-appId>`.
...
8. MCP client sends token to persona-sati MCP. `JWTValidator` accepts because `aud` matches and `azp` is in the allow-list.
9. At 12h, MCP client uses refresh token silently. No browser, no session restart.

This is exactly Microsoft's recommended pattern from [interactive-agent-authentication-authorization-flow](https://learn.microsoft.com/en-us/entra/agent-id/identity-platform/interactive-agent-authentication-authorization-flow), adapted for the Claude-Code-as-MCP-client scenario. Estimated cost (per persona-sati engineering-status): 2–3 commits + one `az ad app create` Brandon runs.
