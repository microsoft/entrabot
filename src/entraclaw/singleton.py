"""Process-singleton lock for entraclaw-mcp.

Background
----------
Every MCP stdio client that attaches to entraclaw spawns a fresh subprocess
(one server per client by spec). Two simultaneous clients on the same
workstation produce two competing entraclaw-mcp processes that race on:
...
- the local interaction log at ``~/.entraclaw/data/interaction_log.jsonl``
...
class SingletonLockHeldError(RuntimeError):
    """Raised when another entraclaw-mcp process already owns the lock.
    ...
    def __init__(self, holder_pid: int | None) -> None:
        ...
        if holder_pid is not None:
            super().__init__(
                f"another entraclaw-mcp instance is already running (PID {holder_pid})"
            )
        else:
            super().__init__("another entraclaw-mcp instance is already running")
...
def default_lock_path() -> Path:
    ...
    Reads ``ENTRACLAW_DATA_DIR`` from the environment via
    :func:`entraclaw.config.get_config`, so tests can monkeypatch the env.
    """
...
def run_or_exit_if_held(path: Path | None = None) -> SingletonLock:
    ...
    On contention writes a one-line ``[entraclaw]`` stderr message naming
    ...
    try:
        ...
        sys.stderr.write(
            f"[entraclaw] another instance is already running (PID {pid_str}); "
