...
> **HISTORICAL — Apr 2026 proposal; work landed in subsequent PRs. Kept for design rationale. See [Engineering Status](../project/status.md) for current state.**
...
- **Multiple Microsoft product groups need it.** GitHub Copilot CLI, ClawPilot, the Maya POC team, Entrabot research, and the digital-teammate workstream are all converging on the same need. Without a Teams-side answer, every team invents its own string-prefix workaround that does not actually solve attribution. With a Teams-side answer, all of them get a consistent customer experience.
