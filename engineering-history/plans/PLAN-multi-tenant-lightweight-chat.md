# PLAN: Multi-Tenant Progressive Identity for EntraBot

> **HISTORICAL — shipped (commit `c8ec521`). Kept for design rationale. See [Engineering Status](../project/status.md) for current state.**

> Generated by `/plan-ceo-review` on 2026-04-09
> Branch: `feature/multi-tenant-lightweight-chat` | Mode: HOLD SCOPE
> Design doc: `~/.gstack/projects/entrabot-identity-research/2026-04-09-feature-multi-tenant-lightweight-chat-design-progressive-identity.md`
> Spec: `docs/architecture/NEXT-WhatsApp-lightweight-teams-chat.md`

## Problem Statement

EntraBot currently requires per-tenant setup (certificates, Blueprint, Agent Identity, Agent User) before an agent can message in Teams. This makes onboarding slow and limits usage to pre-configured tenants.

**Goal:** Any user from any Entra ID tenant can start messaging through EntraBot immediately using their own identity (delegated token), with automatic upgrade to a dedicated Agent User identity when provisioning infrastructure exists.
...
- `DELEGATED` — human's token via MSAL, messages prefixed `[EntraBot]`
...
src/entrabot/
  ...
  tools/teams.py          — [EntraBot] prefix, sent-message dedup, empty validation
...
| 4 | Dedicated research tenant, tenant-agnostic design | Config not code |
...
| 7 | [EntraBot] prefix for research | Sufficient for research attribution |
...
| 17 | ENTRABOT_SKIP_PROVISIONING override | For testing Phase 1 in isolation |
...
```
  ...
  State: DELEGATED
    ...
    ├─► Messages prefixed [EntraBot]
    ├─► Audit: attribution_type = "delegated-human"
    │
    ├─► If ENTRABOT_PROVISIONER_CLIENT_ID set AND SKIP_PROVISIONING != true:
    │     │
    │     ▼
...
2. Write `ENTRABOT_CLIENT_ID` to `.env`

### Provisioner App (PR #2)
1. Create provisioner app registration (application permissions for Agent Identity APIs)
2. Generate client secret
3. Write `ENTRABOT_PROVISIONER_CLIENT_ID` and `ENTRABOT_PROVISIONER_CLIENT_SECRET` to `.env`

## .env Changes

```bash
# PR #1: Multi-tenant delegated auth
ENTRABOT_CLIENT_ID=<multi-tenant app ID>
ENTRABOT_TENANT_ID=<"common" for multi-tenant discovery>
ENTRABOT_SKIP_PROVISIONING=false  # set true to force delegated-only

# PR #2: Provisioner (optional)
ENTRABOT_PROVISIONER_CLIENT_ID=<provisioner app ID>
ENTRABOT_PROVISIONER_CLIENT_SECRET=<provisioner secret>
